// Port do engine My_Blog_Makes_Neil_Proud. Cron diário (vercel.json) + chamada manual.
export const maxDuration = 300; // Vercel Pro — até 300s para pipeline completo
export const dynamic = 'force-dynamic';

import { NextRequest, NextResponse } from 'next/server';
import { revalidatePath } from 'next/cache';
import { claimBlogRunToday, insertArticle, insertRunLog } from '@/lib/blog/supabase-blog';
...
export async function GET(request: NextRequest) {
  ...
  try {
    // 1. Keyword do dia (GSC desligado nesta instalação — seeds do profile)
    keyword = await fetchTopKeyword();
